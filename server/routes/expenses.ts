...
export const expensesRoute = new Hono()
  ...
  .post('/', zValidator('json', createExpenseSchema), async (c) => {
    ...
    const expense = await withSignedDownloadUrl(created)
  ...
  .put('/:id{\\d+}', zValidator('json', updateExpenseSchema), async (c) => {
    ...
    if (Object.keys(updates).length === 0) return c.json({ error: 'Empty update' }, 400)
    const [updated] = await db.update(expenses).set(updates).where(eq(expenses.id, id)).returning()
    if (!updated) return c.json({ error: 'Not found' }, 404)
    const expense = await withSignedDownloadUrl(updated)
    return c.json({ expense })
  })
  .patch('/:id{\\d+}', zValidator('json', updateExpenseSchema), async (c) => {
    ...
    if (Object.keys(updates).length === 0) return c.json({ error: 'Empty patch' }, 400)
    const [updated] = await db.update(expenses).set(updates).where(eq(expenses.id, id)).returning()
    if (!updated) return c.json({ error: 'Not found' }, 404)
    const expense = await withSignedDownloadUrl(updated)
    return c.json({ expense })
  ...
  .delete('/:id{\\d+}', async (c) => {
    ...
    const deleted = await withSignedDownloadUrl(deletedRow)
